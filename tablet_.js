...
app.whenReady().then(() => {
const devices = HID.devices();
tablet = new HID.HID(devices.find(device => device.vendorId === ... && device.productId === ...).path);

tablet.on('data', (data) => {
// Traite les données du stylet pour déplacer le curseur ou effectuer d'autres actions
// ...
});
