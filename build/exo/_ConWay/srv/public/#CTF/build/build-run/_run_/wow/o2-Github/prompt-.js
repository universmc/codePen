...
// Execute the Github model prompting
async function executePromptingGithub() {
  try {
    console.log("Starting Github content generation task...");
    ...
    const filePath = `output/Github_Pibot_${Date.now()}.md`;
    fs.writeFileSync(filePath, content || "No content generated.");
    console.log(`Github content saved to ${filePath}`);
  } catch (error) {
