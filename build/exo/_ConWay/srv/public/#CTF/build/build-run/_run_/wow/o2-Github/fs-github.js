...
async function executeAndMatchAIModels() {
  ...
  const match = {GITHUB: null, GROQ: null, GEMINI: null,HTML: null,CSS: null,CSS: null,JS: null,JSON: null,SVG: null,DRAWIO: null  }; // Initialize match results

  try {

    // Execute prompt --Github content generation task
    console.log("Starting Groq-html content generation task...");
    const GithubCompletion = await groq.chat.completions.create({
      messages: [
        { role: "assistant", content: JSON.stringify(dev) },
        { role: "user", content: JSON.stringify(promptingGithub) }
      ],
      model: "gemma2-9b-it",
      temperature: 0.7
    });

    const GithubContent = GithubCompletion.choices[0]?.message?.content;
    const GithubFilePath = `data/Github_${Date.now()}.md`;
    fs.writeFileSync(GithubFilePath, GithubContent || "No content generated.");
    console.log(`groq content saved to ${GithubFilePath}`);
    match.GITHUB = GithubContent ? "Success" : "Failure";
  ...
  console.log(`Documentation Generation: ${match.GITHUB}`);
