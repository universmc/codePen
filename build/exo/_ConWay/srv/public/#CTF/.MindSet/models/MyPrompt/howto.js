...
const prompt = process.argv[2] || 'system_HOWTO_MY_PROMPT'; //  Obtenir le sujet via l'argument de ligne de commande

const prompting = [
     "PROMPT_GROQ_HOWTO",
     "PROMPT_GROQ_HOWTO",
     "PROMPT_GROQ_HOWTO+role_system",
     "PROMPT_GROQ_HOWTO+role_assistant",
     "PROMPT_GROQ_HOWTO+role_user",
     "PROMPT_GROQ_HOWTO+role_user",

];

async function main() {
  for (const prompt of prompting) {
    try {
      const completion = await groq.chat.completions.create({
      messages: [
        {role: "system", content:"Phase 0: initialisation de `const nodule of nodeJs;`"},
        { role: "user", content: `${prompting}`  },
          {role: "assistant", content:`Imaginer un écosystème nodulaire (node_modules)[ PROMPTO -ia "PROMTP HOWTO ","prompt_howto-nodule_Prompt","prompt_howto-nodule_next","howto-nodule_prompt","prompto-howto-nodule_groq-","howto-nodule_groq-mixtral-8x7b-32768","howto-nodule_node-Makefile"] avec intersections logiques et réseau neuronal
      ...
      const outputFilePath = `data/make_${prompt}_` + new Date().toISOString().replace(/[-:TZ]/g, "") + ".md";
fs.writeFileSync(outputFilePath, mdContent);
      console.log(`Le How-To sur ${prompt} a été enregistrée sur github dans ${outputFilePath}`);       
    });
  } catch (error) {
