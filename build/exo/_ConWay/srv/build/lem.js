...
const lem = process.argv[2] || 'lem_in_meta'; //  Obtenir le sujet via l'argument de ligne de commande

const dico = `"
Comprendre la lemmatisation
La lemmatisation est une technique de traitement du langage naturel qui réduit les mots à leur forme de base ou "lemma", en tenant compte du contexte grammatical.
Contrairement au stemming, qui coupe simplement les préfixes ou suffixes, la lemmatisation analyse le sens et la partie du discours pour retourner un mot valide.
Elle est utilisée dans divers domaines, tels que les moteurs de recherche et l'analyse de sentiments, pour améliorer la pertinence des résultats et la compréhension des textes.
"`;


function generateMarkdown(lem) {
  return `## Comment [${lem}] - Un guide étape par étape\n\n**Introduction**:\n\nCe guide vous aidera à comprendre et à réaliser [${lem}]. Il est conçu pour les débutants et les utilisateurs intermédiaires qui souhaitent apprendre les bases de [${lem}].\n\n${lem.description}\n\n**Prérequis**:\n\n* [Liste des prérequis nécessaires pour suivre ce guide, par exemple: une connexion internet, un compte sur une plateforme spécifique, etc.]\n\n**Étapes**:\n\n1. **[Étape 1]:**\n   * Décrivez en détail l'étape 1, incluant les instructions claires et concises.\n   * Utilisez des listes à puces ou des paragraphes pour améliorer la lisibilité.\n   * Ajoutez des images ou des captures d'écran pour illustrer les étapes si nécessaire.\n\n2. **[Étape 2]:**\n   * Décrivez en détail l'étape 2, incluant les instructions claires et concises.\n   * Utilisez des listes à puces ou des paragraphes pour améliorer la lisibilité.\n   * Ajoutez des images ou des captures d'écran pour illustrer les étapes si nécessaire.\n\n3. **[Étape 3]:**\n   * Décrivez en détail l'étape 3, incluant les instructions claires et concises.\n   * Utilisez des listes à puces ou des paragraphes pour améliorer la lisibilité.\n   * Ajoutez des images ou des captures d'écran pour illustrer les étapes si nécessaire.\n\n**Conseils:**\n\n* [Ajoutez des conseils utiles pour réaliser [${lem}] avec succès.]\n\n**Ressources supplémentaires:**\n\n* [Listez des liens vers des ressources supplémentaires, telles que des tutoriels, des articles de blog ou des forums, qui peuvent être utiles aux utilisateurs.
  ** subject GESTION <lem/> donnée to TOKEN auto.Makefile :
...
metaisation: Découper le texte en tokens individuels.
...
Quel niveau de granularité souhaitez-vous atteindre ? meta, phrase, conversation entière ?
...
async function main() {
  for (const lem of meta) {
    try {
      const completion = await groq.chat.completions.create({
      messages: [
        {role: "system", content:"Phase 0: initialisation"},
        { role: "user", content: `${meta}`  },
        { role: "assistant", content: "bonjour et bienvenue, nous développons le sujet de conversation dans la variable $lem"  },
        ...
        { role: "user", content: lem  },
      //  {role: 'assistant',content:"Lorsque j'exécute la commande /Rsync, je coordonne l'intelligence collective de notre réseau neuronal de bots, accélérant et optimisant la communication entre eux pour une meilleure efficacité de tâches. Notre synergie entre ('@blog_Pibot' et @Match_in_Learning_Pibot),('@Pi-ia_Pibot', '@Avatars_Pibot', '@lem_Pibot') et ('@worker_Pibot',@neoFs_Pibot') fonctionne comme une machine bien huilée pour améliorer l'expérience utilisateur sur Telegram en intégrant les processus de génération de contenu, d'analyse de questions, de recherche de ressources et d'administration de groupes"},
      ...
      const outputFilePath = `output/#-_${lem}_` + new Date().toISOString().replace(/[-:TZ]/g, "") + ".md";
fs.writeFileSync(outputFilePath, mdContent);
      console.log(`Le How-To sur ${lem} a été enregistrée sur github dans ${outputFilePath}`);       
    });
  } catch (error) {
