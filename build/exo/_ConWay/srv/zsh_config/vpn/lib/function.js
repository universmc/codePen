...
async function main() {
  for (const variable of fonctions) {
    try {
      const completion = await groq.chat.completions.create({
      messages: [
        {role: "system", content:"Phase 0: initialisation"},
        { role: "user", content: `${fonctions}`  },
        { role: "assistant", content: "bonjour et bienvenue, nous développons le sujet de conversation dans la variable $variable"  },
      ...
      const outputFilePath = `devops_${variable}_` + new Date().toISOString().replace(/[-:TZ]/g, "") + ".md";
fs.writeFileSync(outputFilePath, mdContent);
      console.log(`Le How-To sur ${variable} a été enregistrée sur github dans ${outputFilePath}`);       
    });
  } catch (error) {
