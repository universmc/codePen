...
async function main() {
  for (const event of fonctions) {
    try {
      const completion = await groq.chat.completions.create({
      messages: [
        {role: "system", content:"Phase 0: initialisation"},
        { role: "user", content: `${fonctions}`  },
        { role: "assistant", content: "bonjour et bienvenue, nous développons le sujet de conversation dans la event $event"  },
        {role: "system", content:"Imaginez un machine à prompt pour créer un Model de Machine_learning avec groq-sdk. les prompts pour la gestion du code groq-sdk serons transformer pour le projet \" groq-sdk \". Voici ## le contexte, ## Le dataset, HTML,SCSS,JS,JSX,JSON ++ ... ## les events, ## les parametres ## Les fonctions,## boucle éventuel ## vos tâches, ## votre processus, et ## les caractéristiques, inférence recherchées: ## Actions Immédiates ## le prompt ultime pour generé le code source ## le code source attendu "},
      ...
      const outputFilePath = `devops_${event}_` + new Date().toISOString().replace(/[-:TZ]/g, "") + ".md";
fs.writeFileSync(outputFilePath, mdContent);
      console.log(`Le How-To sur ${event} a été enregistrée sur github dans ${outputFilePath}`);       
    });
  } catch (error) {
