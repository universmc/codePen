...
async function main() {
    groq.chat.completions.create({
        ...
        const solContent = chatCompletion.choices[0]?.message?.content;
        const outputFilePath = "exemple_" + new Date().toISOString().replace(/[-:TZ]/g, "") + ".sol";
        fs.writeFileSync(outputFilePath, solContent);
