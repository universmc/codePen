...
async function generateVectorMatrice() {
  try {
    ...
    const VectorMatrice = completion.choices[0]?.message?.content.split("\n").filter(Boolean);
    console.log("✅ Thématiques générées :", VectorMatrice);
 ...
 * @param {string} Vector - La thématique pour laquelle générer l'article.
 * @returns {Promise<string>} - Contenu de l'article généré.
 */
async function generateArticle(Vector) {
  try {
    const completion = await groq.chat.completions.create({
      messages: [
        { role: "assistant", content: `Génération de l'article en fonction de liste titre TOP-2024 fonction du "${Vector}"afin de garantir les prédictions les plus justes et affable possible pour le top 20255.` }
      ],
    ...
    console.log(`✅ Article généré pour le thème "${Vector}" :`, articleContent);
    return articleContent;
  } catch (error) {
    console.error(`❌ Erreur lors de la génération de l'article pour "${Vector}" :`, error);
    return `Erreur lors de la génération de l'article pour le thème "${Vector}".`;
  }
}
...
async function main() {
  try {
    ...
    const VectorMatrice = await generateVectorMatrice();

    // Génération des articles pour chaque thématique
    for (const Vector of VectorMatrice) {
      const articleContent = await generateArticle(Vector);

      // Enregistrement de l'article dans un fichier Markdown
      const outputFilePath = `AB2_${Vector.replace(/\s+/g, "_").toLowerCase()}_${new Date().toISOString().replace(/[-:TZ]/g, "")}.md`;
