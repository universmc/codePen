...
/**
 * Génère un ThreeJs complet en fonction d'une thématique via le rôle assistant.
 * @param {string} ThreeJs - La thématique pour laquelle générer l'ThreeJs.
 * @returns {Promise<string>} - Contenu de l'ThreeJs généré.
 */
async function generateArticle(ThreeJs) {
  try {
    const completion = await groq.chat.completions.create({
      messages: [
        { role: "assistant", content: `Génération de l'ThreeJs en fonction de liste titre TOP-2024 fonction du "${ThreeJs}"afin de garantir les prédictions les plus justes et affable possible pour le top 20255.` }
      ],
    ...
    const ThreeJsCanva = completion.choices[0]?.message?.content;
    console.log(`✅ Article généré pour le thème "${ThreeJs}" :`, ThreeJsCanva);
    return ThreeJsCanva;
  } catch (error) {
    console.error(`❌ Erreur lors de la génération de l'ThreeJs pour "${ThreeJs}" :`, error);
    return `Erreur lors de la génération de l'ThreeJs pour le thème "${ThreeJs}".`;
  }
}

/**
 * Fonction principale pour orchestrer la génération des thématiques et des ThreeJss.
 */
async function main() {
  try {
    ...
    // Génération des ThreeJss pour chaque thématique
    for (const ThreeJs of VertexAi) {
      const ThreeJsCanva = await generateArticle(ThreeJs);

      // Enregistrement de l'ThreeJs dans un fichier Markdown
      const outputFilePath = `Canva_${ThreeJs.replace(/\s+/g, "_").toLowerCase()}_${new Date().toISOString().replace(/[-:TZ]/g, "")}.md`;
      fs.writeFileSync(outputFilePath, ThreeJsCanva);

      console.log(`✅ : Canvas : sauvegardé : ${outputFilePath}`);
    }
  } catch (error) {
