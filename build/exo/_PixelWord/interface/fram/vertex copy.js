 ...
 */
async function generateTensors(VectorMatrice) {
  try {
    const completion = await groq.chat.completions.create({
      messages: [
        { role: "assistant", content: `Génération de l'embedding en fonction de liste titre TOP-2024 fonction du "${VectorMatrice}"afin de garantir les prédictions les plus justes et affable possible pour le top 20255.` }
      ],
    ...
    const embeddingContent = completion.choices[0]?.message?.content;
    console.log(`✅ Tensors généré pour le thème "${VectorMatrice}" :`, embeddingContent);
...
async function main() {
  try {
    ...
    for (const VectorMatrice of VertexAi) {
      const embeddingContent = await generateTensors(VectorMatrice);

      // Enregistrement de l'embedding dans un fichier Markdown
      const outputFilePath = `AB2_${VectorMatrice.replace(/\s+/g, "_").toLowerCase()}_${new Date().toISOString().replace(/[-:TZ]/g, "")}.md`;
      fs.writeFileSync(outputFilePath, embeddingContent);
