 ...
 * @param {string} VectorMatrice - La thématique pour laquelle générer l'Embedding.
 * @returns {Promise<string>} - Contenu de l'Embedding généré.
 */
async function generateEmbedding(VectorMatrice) {
  try {
    const completion = await groq.chat.completions.create({
      messages: [
        { role: "assistant", content: `Génération de l' en fonction de liste titre TOP-2024 fonction du "${VectorMatrice}"afin de garantir les prédictions les plus justes et affable possible pour le top 20255.` }
      ],
    ...
    const EmbeddingCanva = completion.choices[0]?.message?.content;
    console.log(`✅ Embedding généré pour le thème "${VectorMatrice}" :`, EmbeddingCanva);
    return EmbeddingCanva;
  } catch (error) {
    console.error(`❌ Erreur lors de la génération de l'Embedding pour "${VectorMatrice}" :`, error);
    return `Erreur lors de la génération de l'Embedding pour le thème "${VectorMatrice}".`;
  }
}
...
async function main() {
  try {
    ...
    for (const VectorMatrice of VertexAi) {
      const EmbeddingCanva = await generateEmbedding(VectorMatrice);

      // Enregistrement de l'Embedding dans un fichier Markdown
      const outputFilePath = `Vector_${VectorMatrice.replace(/\s+/g, "_").toLowerCase()}_${new Date().toISOString().replace(/[-:TZ]/g, "")}.md`;
      fs.writeFileSync(outputFilePath, EmbeddingCanva);

      console.log(`✅ Vector Matriciel 'Embedding' sauvegardé : ${outputFilePath}`);
    }
  } catch (error) {
